...
class MobileNetV3_small(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.bneck3 = BottleNeck(24, 40, 2, 96, activation="HS", use_se=True, ksize=5)
        self.bneck4 = BottleNeck(40, 96, 1, 576, activation="HS", use_se=True, ksize=5)
        self.conv2 = nn.Sequential(nn.Conv2d(96, 2, 1, stride=1, padding=0),
                                   nn.BatchNorm2d(2),
                                   Hswish())
        self.globelavgpool = nn.AdaptiveAvgPool2d((1,1))
    def forward(self, inp):
        x = self.conv1(inp)
        x = self.bneck1(x)
        x = self.bneck2(x)
        ...
        x = self.bneck4(x)
        x = self.conv2(x)
        x = self.globelavgpool(x)
        x = x.view(-1, 2)  # output(32*5*5)
        return x
